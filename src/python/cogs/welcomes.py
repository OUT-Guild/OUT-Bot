...
class Welcomes(commands.Cog):
    ...
    async def on_member_join(self, member):
        if not member.bot:
            ...
            await channel.send(
                f":boom::boom:Welcome to OUT! {member.mention}឵, you're member number {member.guild.member_count}, Remember to read the rules. Have fun!:boom::boom:"
            )
    ...
    async def link(self, ctx, username):
        ...
        if user_collection.find({"uuid": uuid}).count() == 0:
            ...
            if hypixel_data["player"]["socialMedia"]["links"]["DISCORD"].lower() != str(ctx.author).lower():
                invalid_embed = self.client.create_embed(
                    "Invalid Discord Tag",
            ...
            if hypixel_data["player"]["networkExp"] < 1305000:
                insufficient_embed = self.client.create_embed(
                    "Insufficient Network Level",
            ...
            if user["_id"] != ctx.author.id:
                claimed_embed = self.client.create_embed(
